<template>
	<view class="ab-page">
		...
		<view v-if="showStatus" class="status-menu">
			<view class="status-item" v-for="s in statuses" :key="s.value" @click="setStatus(s.value)">{{ s.label }}
			</view>
		</view>
		...
		<scroll-view class="list" scroll-y>
			<view class="card" v-for="item in filteredData" :key="item.id" @click="openDetail(item)">
				...
				<view class="card-body">
					<view class="field"><text class="label">异常工序：</text><text class="value">{{ item.process }}</text>
					</view>
					<view class="field"><text class="label">异常分类：</text><text class="value">{{ item.category }}</text>
					</view>
					<view class="field"><text class="label">异常时间：</text><text class="value">{{ item.time }}</text>
						<text class="value" style="color: #5b4bff;">{{ item.statusText }}</text>
					</view>
				</view>
			</view>
		</scroll-view>
...
const data = reactive([
	...
	{ id: 2, code: 'YCBH00000002', process: '第三道工序', category: '质量异常', time: '2025-04-22 10:12:00', status: 'unhandled', statusText: '未处理' },
...
function openDetail(item) {
	// 将整条记录通过路由参数传递给详情页，便于直接回显
	const itemStr = encodeURIComponent(JSON.stringify(item))
	uni.navigateTo({ url: `/pages/abnormal/detail?item=${itemStr}` })
}
