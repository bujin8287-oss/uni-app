...
function openPicker(key) {
	if (key === 'startDate') {
		...
		// 跳转到生产厂家选择页面
		uni.navigateTo({ 
			url: `/pages/workbench/manufacturer-select/index?current=${encodeURIComponent(formData.manufacturer || '')}` 
		})
	} else if (key === 'department') {
		// 跳转到部门选择页面
		uni.navigateTo({ 
			url: `/pages/workbench/department-select/index?current=${encodeURIComponent(formData.department || '')}` 
		})
	}
}
